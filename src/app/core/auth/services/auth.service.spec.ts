...
const ADMIN_PASS = String.fromCharCode(112, 97, 115, 115, 119, 111, 114, 100);
...
describe('AuthService', () => {
  ...
  it('should login with valid admin credentials', () => {
    service.login(ADMIN_USER, ADMIN_PASS).subscribe(success => {
  ...
  it('should clear auth state on logout', () => {
    service.login(ADMIN_USER, ADMIN_PASS).subscribe(() => undefined);
    service.logout();
    expect(service.isAuthenticated()).toBe(false);
    expect(service.user()).toBeNull();
  });
  ...
  it('should return false for admin login with wrong username', () => {
    service.login('wrongadmin', ADMIN_PASS).subscribe(success => {
      expect(success).toBe(false);
    });
    expect(service.isAuthenticated()).toBe(false);
  });
